...
spine, a road+wire grid, two separated neighborhoods, and an obs-reactive
builder that grows zones next to whatever it has already powered). Under the
`res_ind` measures (= res_share, ind_share of zoned tiles) these land in
distinct cells, giving MAP-Elites a broad, non-degenerate seed front and the
...
# --------------------------------------------------------------------------
# Obs-REACTIVE builder: instead of a fixed plan, it lays a power+road spine for
# the first chunk of steps, then for the rest of the episode reads obs.tile_map
# and drops a new RESIDENTIAL zone next to an already-powered wire tile,
# growing the city outward from what is already working. A genuinely
# closed-loop policy (uses obs), unlike the open-loop spine seeds.
# --------------------------------------------------------------------------
SEED_REACTIVE = '''
def act(obs, state):
    cx, cy = WORLD_W // 2, WORLD_H // 2
